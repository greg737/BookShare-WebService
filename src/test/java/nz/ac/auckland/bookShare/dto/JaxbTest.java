...
public class JaxbTest {
	...
	public static void setUpJAXB() throws JAXBException, IOException, SAXException {
		_jaxbCxt = JAXBContext.newInstance(User.class, Book.class, Author.class, Location.class, Request.class);
		...
		// directory.
		SchemaOutputResolver sor = new DefaultSchemaOutputResolver(OUTPUT_PATH, XML_FILE_SCHEMA);
		...
		Schema schema = factory.newSchema(new File(OUTPUT_PATH + XML_FILE_SCHEMA));
	...
	public void initialiseFixtureObjects() {
		_user = new User("bookLover1", "Jess", "A", "Auckland");

		_author = new Author("James", "James");
		
		_book = new Book("How to program", Genre.COMEDY, "English", Type.PAPERBACK, _author.getId());
		
		_author.addNewWritten(_book);
		_author.addNewWritten(new Book("How to webservice", Genre.COMEDY, "English", Type.PAPERBACK, _author.getId()));

		

		_location = new Location(12.34, 62.3);

		_request = new Request(_user, _book);
	}
	...
	public void marshalUser() throws JAXBException {
		_marshaller.marshal(_user, System.out);
	}

	@Test
	public void marshalAndUnmarshalUser() throws JAXBException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		_marshaller.marshal(_user, baos);
		
		ByteArrayInputStream bais = new ByteArrayInputStream(baos.toByteArray());

		// Unmarshal the Library.
		User copy = (User) _unmarshaller.unmarshal(bais);

		// The two Book instances should have the same value (i.e. state).
		assertEquals(_user, copy);

		// The two Book instance are separate objects.
		assertNotSame(_user, copy);
	}
	
	@Test
	public void marshalAuthor() throws JAXBException {
		_marshaller.marshal(_user, System.out);
	}
	...
	public void marshalAndUnmarshalLibraryToFromFile(){
		...
		try {
			_marshaller.marshal(_author, xmlFile);

			// Unmarshal the Library.
			Author copy = (Author) _unmarshaller
					.unmarshal(xmlFile);
	...
	@Test
	public void marshalRequest() throws JAXBException {
		_marshaller.marshal(_request, System.out);
	}
	 ...
	 */
	@Test
	public void marshalAndUnmarshalRequest() throws JAXBException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		_marshaller.marshal(_request, baos);

		ByteArrayInputStream bais = new ByteArrayInputStream(baos.toByteArray());
		Request copy = (Request) _unmarshaller.unmarshal(bais);

		// The two Book instances should have the same value (i.e. state).
		assertEquals(_request, copy);

		// The two Book instance are separate objects.
		assertNotSame(_request, copy);
	}
	
	@Test
	public void marshalLocation() throws JAXBException {
		_marshaller.marshal(_location, System.out);
	}
	 ...
	 */
	@Test
	public void marshalAndUnmarshalLocation() throws JAXBException {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		_marshaller.marshal(_location, baos);

		ByteArrayInputStream bais = new ByteArrayInputStream(baos.toByteArray());
		Location copy = (Location) _unmarshaller.unmarshal(bais);

		// The two Book instances should have the same value (i.e. state).
		assertEquals(_location, copy);

		// The two Book instance are separate objects.
		assertNotSame(_location, copy);
	}
}
